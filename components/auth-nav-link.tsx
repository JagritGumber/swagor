...
/**
 * Pathname-aware nav cluster.
 *   - Logged out: cyan "Sign In" -> /sign-in
 *   - Logged in, NOT on /dashboard: cyan "Dashboard" link side-by-side with a
 *     small initials dropdown for Profile / Sign Out (one-click to dashboard).
 *   - Logged in, already on /dashboard*: just the initials dropdown.
 */
export function AuthNavLink() {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger className={DROP_TRIGGER} aria-label="Account menu">
          <span>{initials}</span>
