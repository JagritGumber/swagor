...
export function buildPerpMarketState(input: Input): PerpMarketState {
  ...
  const dataQuality = price === null || candles.length < 10 ? "degraded" : qualities.includes("unknown") ? "partial" : "complete";
  ...
  const permission = choosePermission({ dataQuality, regime, valueLocation, auctionState, structureState, flow: derivativesFlow.flowRead });
  const setupCandidates = price === null ? [] : candidates(input.symbol, permission, auctionState, structureState, valueLocation, levels, derivativesFlow.flowRead);
  return {
...
function choosePermission(s: Pick<PerpMarketState, "dataQuality" | "regime" | "valueLocation" | "auctionState" | "structureState"> & { flow: PerpMarketState["derivativesFlow"]["flowRead"] }): PerpPermission {
  if (s.dataQuality === "degraded" || s.regime === "volatile") return "avoid_new_risk";
...
function candidates(symbol: string, permission: PerpPermission, auction: PerpMarketState["auctionState"], structure: PerpMarketState["structureState"], location: PerpMarketState["valueLocation"], l: PerpMarketState["levels"], flow: PerpMarketState["derivativesFlow"]["flowRead"]): PerpSetupCandidate[] {
