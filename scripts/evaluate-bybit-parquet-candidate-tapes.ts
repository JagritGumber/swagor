...
for (const month of monthRange(startMonth, endMonth)) {
  ...
  if (!force && await hasValidTape(tapePath, month)) {
    console.log(`SKIP candidate month=${month} out=${tapePath}`);
    continue;
  }
...
async function runEvaluator(input: {
  ...
  if (exitCode !== 0) throw new Error(`candidate evaluation failed month=${input.month}: ${stderr || stdout}`);
  const candidateLine = stdout.match(/candidate_tape=(.+)/);
  console.log(candidateLine ? `DONE ${input.month} ${candidateLine[0]}` : `DONE ${input.month}`);
}
