...
async function runWithFilter(label: string, entryFilter?: { requireKnownRegime?: boolean; requireExtremeAuction?: boolean; maxStopDistancePct?: number }) {
...
async function main() {
  console.log('=== Benchmark comparison: raw vs filtered ===\n')

  await runWithFilter('RAW (no filter)')
  await runWithFilter('FILTER 1: requireKnownRegime', { requireKnownRegime: true })
  await runWithFilter('FILTER 2: requireExtremeAuction', { requireExtremeAuction: true })
  await runWithFilter('FILTER 3: maxStopDistancePct 0.002 (0.2%)', { maxStopDistancePct: 0.002 })
  await runWithFilter('FILTER ALL: regime + extreme + tight stop', {
