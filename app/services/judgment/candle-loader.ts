...
export async function loadCandlesForAsset(
  ...
  let rawCandles: HyperliquidCandle[];
  try {
    rawCandles = (await hooked.send()) as HyperliquidCandle[];
  } catch (err) {
    throw new Error(
      `Failed to load candles for ${asset}: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
