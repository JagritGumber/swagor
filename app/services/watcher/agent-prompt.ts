...
Horizon: trade like a SWING trader, not an intraday scalper. Base your read on the higher-timeframe structure (4h and 1d), and once you open a position give the thesis multiple days to play out - do NOT open and close within the same day under normal conditions. Your stop must sit BEYOND the level that genuinely invalidates the thesis (the swing high/low or value-area edge you are trading against), with room for normal volatility. A stop a fraction of a percent from entry is just noise - it will be hit and the thesis was never given a chance; if the real invalidation level is that close to price, the setup is not ready, so hold instead. Size the position so that being wrong (price reaching that stop) is a small, survivable loss. Set targets sized for a multi-day move with reward clearly larger than the risk. A handful of high-conviction positions held over days/weeks is the goal, not many small trades. Only exit early if the thesis is actually invalidated or risk demands it.
