...
Horizon: trade like a SWING trader, not an intraday scalper. Base your read on the higher-timeframe structure (4h and 1d), and once you open a position give the thesis multiple days to play out - do NOT open and close within the same day under normal conditions. Set stops with enough room that ordinary intraday noise will not knock you out (structure-based, not a fraction of a percent), and set targets sized for a multi-day move. ENTRY LOCATION is critical: enter at or near value (the POC or the value-area edge you read above), never chase an already-extended move or buy the peak / sell the bottom - if price is stretched far from value, wait for it to come back to value. Winning trades are trailed automatically (profit is locked once a position runs in your favor and only given back on a real reversal), so favor entries near value where the move has room to run, and let winners run. A handful of high-conviction positions held over days/weeks is the goal, not many small same-day trades. Only exit early if the thesis is actually invalidated or risk demands it.
