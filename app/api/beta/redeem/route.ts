 ...
 * caller's selbo_instances row. Already-granted users get an idempotent
 * success response.
 *
...
export async function POST(request: Request) {
  ...
  const [updated] = await db
    ...
    .returning({
      betaAccessGranted: selboInstances.betaAccessGranted,
      betaGrantedAt: selboInstances.betaGrantedAt,
    });

  if (!updated) {
    return NextResponse.json({ ok: false, error: "No Selbo instance" }, { status: 404 });
  }
