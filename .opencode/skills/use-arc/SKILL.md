...
## Connect Wallet — Arc Docs Reference Pattern

The Arc docs `ConnectWallet` component uses `window.__arcWallet` directly with a simple state machine:

```tsx
const [status, setStatus] = useState('checking')
// checking → not-installed | disconnected | connected

// On mount: check window.__arcWallet
// undefined → not-installed
// exists + ready + no state → disconnected
// exists + has state → connected
```

No `window.ethereum` fallbacks. No `Record<string, unknown>` casts. No cached bus. Direct reads of `window.__arcWallet`.

- `ARC_TESTNET_CHAIN_ID`: `5042002` (`0x4CEF52` — note this differs from the chain hex `0x4CF4B2` used in EIP-3085 `wallet_addEthereumChain` calls)
- `switchChain(chainId)`: Switch the wallet's active network
- `wallets`: Array of `{ info: { uuid, name, icon } }` for multi-wallet picker

Error codes:
- `code: 'MULTIPLE_WALLETS'` — multiple wallets detected, show wallet picker
- `code: 4001` — user rejected the request
- `message: /No wallet detected/i` — no wallet found
- `message: /User cancelled/i` — user cancelled
